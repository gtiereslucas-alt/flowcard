...
await card.deployToNFC();`;
  ...
  const renderCodeLine = (line: string, i: number) => {
    let coloredLine = <span className="text-slate-400">{line}</span>;
    if (line.startsWith("//")) {
      ...
      coloredLine = (
        <span>
            {parts.map((p, idx) => 
                ['const ', 'await ', 'new '].includes(p) ? <span key={idx} className="text-purple-400 font-bold">{p}</span> : 
                p.includes('FlowCardDesign') ? <span key={idx} className="text-yellow-300">{p}</span> :
                <span key={idx} className="text-slate-300">{p}</span>
            )}
        </span>
      );
  ...
  return (
    <section ref={containerRef} className="py-24 md:py-48 relative overflow-hidden bg-background border-t border-border-custom">
      ...
      <div className="container mx-auto px-6">
        <div className="flex flex-col items-center text-center mb-24">
            <FadeIn>
                ...
                <h2 className="text-6xl md:text-[5.5rem] font-black tracking-tighter mb-8 leading-[0.85]">
                    L'Excellence <br />
        ...
        <FadeIn delay={0.2} className="max-w-6xl mx-auto w-full mb-32 relative group">
            ...
            <motion.div 
                className="relative bg-[#0a0a0b] rounded-[3rem] border border-white/10 shadow-2xl overflow-hidden flex flex-col lg:flex-row transform-gpu"
            >
                <div className="w-full lg:w-[55%] flex flex-col border-b lg:border-b-0 lg:border-r border-white/10">
                    <div className="flex items-center gap-2 px-8 py-5 bg-white/5 border-b border-white/5">
                ...
                <div className="w-full lg:w-[45%] bg-[#111] relative overflow-hidden flex items-center justify-center p-20 min-h-[500px]">
                    ...
                    <div className="absolute top-8 right-8 flex items-center gap-2 px-4 py-2 rounded-full bg-black/50 border border-white/10 backdrop-blur-md">
                        <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
                        <span className="text-[10px] font-black text-white uppercase tracking-widest">Real-time render</span>
                    </div>
                    ...
                    <motion.div 
                        animate={{ 
                            ...
                            rotateX: 10,
                            boxShadow: hasColor ? "0 30px 60px -15px rgba(37,99,235,0.4)" : "0 30px 60px -15px rgba(0,0,0,0.4)"
                        }}
                        ...
                        className={`relative w-[280px] aspect-[1.58/1] rounded-3xl border flex flex-col justify-between p-8 transform-gpu transition-all duration-700
                            ${hasMaterial ? 'bg-surface border-white/20' : 'bg-white/5 border-white/10 border-dashed'}
                        `}
                    >
                        <div className={`absolute inset-0 bg-gradient-to-br from-white/10 to-transparent mix-blend-overlay rounded-3xl transition-opacity duration-1000 ${hasMaterial ? 'opacity-100' : 'opacity-0'}`} />
                        ...
                        <div className="relative z-10 w-full space-y-4">
                            <div className="h-2 w-10 rounded-full bg-white/10 overflow-hidden">
                                <motion.div 
                                    initial={{ width: 0 }}
                                ...
                                <AnimatePresence>
                                    {hasText && (
                                        <motion.span 
                                            ...
                                            className="text-xs font-black uppercase tracking-[0.3em] text-white/90 drop-shadow-md block"
                                        >
                                            Lucas Gomes T.
                                        </motion.span>
                                    )}
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
            <FadeIn delay={0.4} className="group p-12 rounded-[3rem] bg-surface border border-border-custom hover:border-accent/30 transition-all duration-500">
                <Sparkles size={40} className="text-accent mb-8 group-hover:scale-110 transition-transform" />
                <h3 className="text-3xl font-black mb-4 text-white">Full Identity Control</h3>
            ...
            <FadeIn delay={0.5} className="group p-12 rounded-[3rem] bg-surface border border-border-custom hover:border-purple-400/30 transition-all duration-500">
                <Layers size={40} className="text-purple-400 mb-8 group-hover:scale-110 transition-transform" />
                <h3 className="text-3xl font-black mb-4 text-white">Ecological Materials</h3>
