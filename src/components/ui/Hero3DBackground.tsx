...
export const Hero3DBackground = ({ children }: { children?: React.ReactNode }) => {
          ...
          <motion.div className="flex flex-col items-center text-center pt-10 md:pt-12 lg:pt-0 lg:items-start lg:text-left relative z-30 w-full px-4 max-w-full">
            <motion.div animate={{ y: [0, -15, 0], rotateX: [0, 5, 0], rotateY: [0, -5, 0] }} transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }}>
              <h1 className={`text-6xl sm:text-8xl md:text-[8rem] lg:text-[5.5rem] xl:text-[7rem] font-black tracking-[-0.04em] leading-[0.85] mb-2 whitespace-pre-wrap break-words transition-colors duration-700 ${isDark ? "text-white drop-shadow-2xl" : "text-[#050505] shadow-none"}`}>
                ...
                <motion.span initial={{ opacity: 0, y: 30, filter: "blur(10px)" }} animate={{ opacity: isDark ? 0.3 : 0.4, y: 0, filter: "blur(0px)" }} transition={{ duration: 0.8, delay: 0.5 }} className="block text-[0.55em] italic tracking-tight font-serif mb-0 whitespace-nowrap">
                  Accélérez vos
                </motion.span>
                <motion.span initial={{ opacity: 0, y: 40, filter: "blur(20px)" }} animate={{ opacity: 1, y: 0, filter: "blur(0px)" }} transition={{ duration: 1.2, delay: 0.7, ease: [0.22, 1, 0.36, 1] }} className={`block pb-2 bg-gradient-to-b text-transparent bg-clip-text ${isDark ? "from-white to-white/40" : "from-[#050505] to-[#050505]/70"}`}>
                  Échanges.
                </motion.span>
              </h1>
