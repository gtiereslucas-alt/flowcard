    ...
    <section className="py-24 md:py-48 relative overflow-hidden bg-[#050505]">
      ...
      <div className="container mx-auto px-6 relative z-10 flex flex-col items-center">
        <div className="max-w-4xl mx-auto text-center mb-32">
            <FadeIn>
                <div className="inline-flex items-center gap-3 px-4 py-2 rounded-full bg-white/5 border border-white/10 text-[10px] font-black uppercase tracking-[0.4em] mb-10 text-white/60">
                    <div className="w-1.5 h-1.5 rounded-full bg-accent animate-pulse" /> Workflow FlowCard v2.0
                </div>
                <h2 className="text-6xl md:text-[6.5rem] font-black tracking-tighter mb-10 leading-[0.85] text-white">
        ...
        <div className="grid lg:grid-cols-3 gap-10 w-full max-w-7xl">
            {steps.map((step, idx) => (
                <FadeIn key={idx} delay={0.1 * idx} direction="up" className="h-full">
                    <motion.div 
                        whileHover={{ y: -10 }}
                        className={`h-full p-12 rounded-[4rem] border border-white/5 bg-gradient-to-br ${step.color} backdrop-blur-xl relative group flex flex-col items-center text-center shadow-2xl overflow-hidden`}
                    >
                        ...
                        {/* Icon Podium */}
                        <div className={`w-24 h-24 ${step.bgAccent} rounded-3xl flex items-center justify-center mb-10 border border-white/5 group-hover:scale-110 group-hover:border-white/20 transition-all duration-500 relative`}>
                        ...
                        <div className="flex flex-wrap gap-2 justify-center">
                            <span className="px-4 py-1.5 rounded-full bg-white/5 border border-white/5 text-[10px] font-bold text-white/40 uppercase tracking-widest">Temps réel</span>
                            <span className="px-4 py-1.5 rounded-full bg-white/5 border border-white/5 text-[10px] font-bold text-white/40 uppercase tracking-widest">Sécurisé</span>
                        </div>
        ...
        <FadeIn delay={0.4} className="mt-24">
             <div className="flex flex-col items-center gap-8">
                ...
                <button className="flex items-center gap-4 text-white hover:text-accent transition-colors font-black text-sm uppercase tracking-[0.3em] group">
