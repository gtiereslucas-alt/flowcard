...
const MaterialCard = ({ title, subtitle, desc, icon: Icon, color, image, features }: any) => (
  <motion.div 
    whileHover={{ y: -10 }}
    className="group relative h-[500px] md:h-[600px] w-full rounded-[4rem] overflow-hidden border border-white/10 bg-[#0a0a0a] shadow-2xl flex flex-col"
  >
    ...
    <div className="relative z-20 h-full p-12 flex flex-col justify-end">
      <div className={`w-16 h-16 rounded-3xl ${color} flex items-center justify-center mb-8 shadow-xl border border-white/10 group-hover:scale-110 transition-transform duration-700`}>
        <Icon size={32} className="text-white" />
      </div>
      <span className="text-accent font-black text-xs uppercase tracking-[0.4em] mb-4">{subtitle}</span>
      <h3 className="text-4xl md:text-5xl font-black text-white mb-6 leading-[1.1] tracking-tighter">{title}</h3>
      <p className="text-text-muted text-lg leading-relaxed mb-8 max-w-sm">{desc}</p>
      
      <div className="flex flex-wrap gap-3">
        {features.map((f: string, i: number) => (
          <span key={i} className="px-4 py-2 rounded-full bg-white/5 border border-white/5 text-[10px] font-bold text-white/50 uppercase tracking-widest">
            {f}
          </span>
        ))}
      </div>
    ...
    <section className="container mx-auto px-8 py-40 overflow-hidden">
      <div className="flex flex-col items-center text-center mb-24">
        <FadeIn>
          ...
          <h2 className="text-5xl md:text-[6rem] font-black tracking-[-0.04em] leading-[0.9] text-white mb-10">
            Matières <br/>
