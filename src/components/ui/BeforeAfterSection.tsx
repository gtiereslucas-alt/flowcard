    ...
    <section className="py-24 md:py-48 bg-zinc-50 dark:bg-[#050505] border-t border-border-custom relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-10">
        <FadeIn className="text-center mb-24">
          <h2 className="text-5xl md:text-8xl font-black tracking-tighter text-foreground dark:text-white mb-8">
            Changez de <br />
            <span className="text-white/20">Dimension.</span>
          </h2>
          <p className="text-zinc-500 dark:text-white/40 text-xl max-w-xl mx-auto font-medium leading-relaxed">
          ...
          <FadeIn delay={0.1}>
            <div className={`group h-full rounded-[3.5rem] border p-12 md:p-16 transition-all duration-700 hover:bg-red-500/[0.08] ${comparison.before.accent}`}>
              <div className="mb-12">
                ...
                <h3 className="text-4xl font-black text-zinc-500 dark:text-white/40 group-hover:text-white/60 transition-colors mb-2">{comparison.before.title}</h3>
              ...
              <div className="space-y-8">
                {comparison.before.items.map((item, idx) => (
                  <div key={idx} className="flex items-center gap-6 group/item">
                    <div className="w-12 h-12 rounded-2xl bg-white/5 border border-border-custom flex items-center justify-center text-white/20 group-hover/item:text-red-400 group-hover/item:border-red-400/20 transition-all">
                      <item.icon size={20} />
                    </div>
          ...
          <FadeIn delay={0.2}>
            <div className={`group h-full rounded-[3.5rem] border p-12 md:p-16 transition-all duration-700 hover:shadow-[0_0_80px_rgba(37,99,235,0.15)] relative overflow-hidden ${comparison.after.accent}`}>
              ...
              <div className="relative mb-12">
                ...
                <h3 className="text-4xl font-black text-foreground dark:text-white mb-2">{comparison.after.title}</h3>
                <p className="text-sm font-bold text-zinc-500 dark:text-white/40">{comparison.after.subtitle}</p>
              </div>
              ...
              <div className="relative space-y-8">
                {comparison.after.items.map((item, idx) => (
                  <div key={idx} className="flex items-center gap-6 group/item">
                    ...
                    <span className="text-foreground dark:text-white font-medium group-hover:translate-x-1 transition-transform">{item.text}</span>
                  </div>
                ))}
              </div>
              
              <div className="mt-12 pt-12 border-t border-zinc-300 dark:border-white/10 relative">
